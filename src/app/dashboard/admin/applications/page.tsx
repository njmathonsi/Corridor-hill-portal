...
export default async function ApplicationsPage() {
  ...
  const { data: apps } = await supabase
    .from('applications')
    .select('id, status, academic_year, submitted_at, financial_status, profiles!applications_student_id_fkey(full_name, student_number, faculty, student_year_type, funding_type)')
    ...
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(apps ?? []).map(app => {
          ...
          return (
            <div key={app.id} style={{ background: '#18181b', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg,#3b82f6,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                {p?.full_name?.split(' ').slice(0,2).map((n: string) => n[0]).join('') ?? '?'}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#fafafa' }}>{p?.full_name ?? '—'}</div>
                <div style={{ fontSize: 11, color: '#71717a', marginTop: 2 }}>{p?.student_number} · {p?.faculty} · {app.academic_year}</div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 99, background: 'rgba(255,255,255,0.06)', color: '#a1a1aa', border: '1px solid rgba(255,255,255,0.08)' }}>
                  {p?.student_year_type === 'first_year' ? '1st Year' : 'Senior'} · {p?.funding_type ?? '—'}
                </span>
                <span style={{ padding: '3px 10px', borderRadius: 99, fontSize: 11, fontWeight: 600, background: c.bg, color: c.color, border: `1px solid ${c.border}` }}>
