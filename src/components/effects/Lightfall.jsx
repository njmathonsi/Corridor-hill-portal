...
uniform float uSpeed; uniform int uStreakCount; uniform float uStreakWidth,uStreakLength,uGlow,uDensity,uTwinkle,uZoom,uBgGlow,uOpacity,uMouseEnabled,uMouseStrength,uMouseRadius;
varying vec2 vUv;
...
void mainImage(out vec4 o, vec2 C){
  ...
  float angRings=max(1.0,floor(6.28318530718*max(uDensity,0.05)+0.5)); vec2 Y=vec2(5e-3,6.28318530718/angRings);
  vec2 c0=sceneC(C,r); vec2 cdx=sceneC(C+vec2(1.0,0.0),r); vec2 cdy=sceneC(C+vec2(0.0,1.0),r);
  vec2 dCx=cdx-c0; vec2 dCy=cdy-c0; dCx.y-=6.28318530718*floor(dCx.y/6.28318530718+0.5); dCy.y-=6.28318530718*floor(dCy.y/6.28318530718+0.5);
  vec2 fw=abs(dCx)+abs(dCy); C=c0;
  vec2 P=vec2(2.0,1.0)*uv0-(r/r.x)*vec2(0.0,1.0); vec4 O=vec4(uBgColor*90.0*uBgGlow/(1e3*dot(P,P)+6.0),0.0);
  float mGlow=0.0;
...
export default function Lightfall({
  ...
  colors=['#A6C8FF','#5227FF','#FF9FFC'], backgroundColor='#0A29FF',
  speed=0.5, streakCount=2, streakWidth=1, streakLength=1, glow=1,
  density=0.6, twinkle=1, zoom=3, backgroundGlow=0.5, opacity=1,
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uOpacity:{value:opacity}, uMouseEnabled:{value:mouseInteraction?1:0}, uMouseStrength:{value:mouseStrength}, uMouseRadius:{value:mouseRadius},
    }
    ...
    const onPointerMove = e => {
      ...
      const x = (e.clientX-rect.left)*scale, y=(rect.height-(e.clientY-rect.top))*scale
    ...
    const loop = t => {
      ...
      if (mouseDampening>0){
        ...
        const tau=Math.max(1e-4,mouseDampening); let factor=1-Math.exp(-dt/tau); if(factor>1)factor=1
        const target=mouseTargetRef.current, cur=uniforms.iMouse.value
    ...
    return () => {
      ...
      const rm=(o,k)=>{ if(o && typeof o[k]==='function') o[k].call(o) }
      rm(programRef.current,'remove'); rm(geometryRef.current,'remove'); rm(meshRef.current,'remove'); rm(rendererRef.current,'destroy')
  ...
  return <div ref={containerRef} className={`lightfall-container ${className??''}`} style={mixBlendMode?{mixBlendMode}:undefined} />
}
