...
export default function RoomExplorer({ blocks, units, rooms, leases, students }: {
  ...
  const unitsInBlock = useMemo(() => {
    if (!selectedBlock) return []
    return units.filter(u => u.block_id === selectedBlock.id).sort((a, b) => a.floor - b.floor || a.unit_code.localeCompare(b.unit_code))
  }, [selectedBlock, units])
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, fontSize: 12, color: '#71717a' }}>
        <span onClick={() => { setSelectedBlock(null); setExpandedUnitId(null) }} style={{ cursor: selectedBlock ? 'pointer' : 'default', color: !selectedBlock ? '#fafafa' : '#71717a', fontWeight: !selectedBlock ? 700 : 400 }}>
          All Blocks
        </span>
        {selectedBlock && (
          <>
            <span>›</span>
            <span style={{ color: '#fafafa', fontWeight: 700 }}>Block {selectedBlock.code}</span>
          </>
        )}
      </div>

      <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>
        {!selectedBlock ? 'Room Management' : `Block ${selectedBlock.code} — Units`}
      </h1>
      <p style={{ fontSize: 13, color: '#71717a', marginBottom: 24 }}>
        {!selectedBlock ? 'Click a block to see its units' : 'Click a unit to reveal its 3 rooms'}
      </p>
      ...
      {!selectedBlock && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 700 }}>
          {blockStats.map(({ block, total, occupied, pct }) => {
            ...
            return (
              <div key={block.id} onClick={() => setSelectedBlock(block)} style={{ background: '#18181b', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '18px 22px', cursor: 'pointer' }}>
      ...
      {selectedBlock && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 700 }}>
          {unitsInBlock.map(unit => {
            ...
            return (
              <div key={unit.id} style={{ background: '#18181b', border: `1px solid ${isExpanded ? 'rgba(59,130,246,0.4)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{
                      ...
                      transition: 'transform 0.2s', color: '#71717a', fontSize: 14,
                    }}>▶</span>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#fafafa' }}>
                        Unit {selectedBlock.code}{unit.unit_code}
                      </div>
                      <div style={{ fontSize: 11, color: '#71717a' }}>Floor {unit.floor} · {total} rooms</div>
                    </div>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            border: `1px solid ${isFull ? 'rgba(244,63,94,0.25)' : 'rgba(16,185,129,0.25)'}`,
                          }}
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
