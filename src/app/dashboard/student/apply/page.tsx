...
export default function ApplyPage() {
  ...
  if (success) return (
    <div style={{ padding: 28, maxWidth: 540 }}>
      <div className="glass-card" style={{ textAlign: 'center', padding: '48px 24px', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.3)' }}>
        ...
        <button onClick={() => router.push('/dashboard/student/home')} className="glass-btn" style={{ padding: '10px 28px', borderRadius: 8, background: '#10b981', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700 }}>Back to Dashboard</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ padding: 28, maxWidth: 560 }}>
      ...
      <div className="glass-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[{val:'first_year',label:'First Year'},{val:'senior',label:'Senior Student'}].map(opt => (
              <button key={opt.val} onClick={() => setYearType(opt.val)} className="glass-btn" style={{ flex: 1, padding: '10px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: yearType === opt.val ? 'rgba(59,130,246,0.15)' : 'transparent', color: yearType === opt.val ? '#3b82f6' : '#71717a', border: `1px solid ${yearType === opt.val ? 'rgba(59,130,246,0.3)' : 'rgba(255,255,255,0.08)'}` }}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 14px', borderRadius: 8, background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)', fontSize: 12, color: '#71717a', lineHeight: 1.6 }}>
          <Info className="w-5 h-5 text-white/70 group-hover:text-white transition-colors" /> Room preferences are noted but management makes the final assignment. All applications are reviewed before approval.
        </div>
        ...
        <button onClick={handleSubmit} disabled={loading} className="glass-btn group" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '12px', borderRadius: 8, background: 'linear-gradient(135deg,#3b82f6,#8b5cf6)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.5 : 1 }}>
