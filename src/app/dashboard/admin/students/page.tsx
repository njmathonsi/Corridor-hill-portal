...
export default async function StudentsPage() {
  ...
  const { data: students } = await supabase
    .from('profiles')
    .select('id, full_name, student_number, email, faculty, course, year_of_study, funding_type, onboarding_complete, is_active')
    .eq('role', 'student')
    .order('full_name')
      ...
      <div style={{ background: '#18181b', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            {['Name', 'Student No.', 'Email', 'Faculty', 'Year', 'Funding', 'Status'].map(h => (
          ...
          <tbody>
            ...
            {(students ?? []).length === 0 && <tr><td colSpan={7} style={{ padding: 32, textAlign: 'center', color: '#52525b' }}>No students registered yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
