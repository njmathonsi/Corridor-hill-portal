...
export default async function BiometricsHubPage() {
  ...
  const { data: regs } = await supabase
    ...
    .select('student_id, biometric_registered, sync_status, registered_at, device_label')
      ...
      <div style={{ background: '#18181b', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          ...
          <tbody>
            ...
            {(students ?? []).length === 0 && (
              <tr>
                <td colSpan={6} style={{ padding: 32, textAlign: 'center', color: '#52525b' }}>No students registered yet.</td>
              </tr>
            )}
          </tbody>
        </table>
